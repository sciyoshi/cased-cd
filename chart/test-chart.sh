...
test_standard_template() {
  ...
  if ! grep -q '^kind: NetworkPolicy$' /tmp/helm-standard.yaml; then
    pass "NetworkPolicy is absent by default"
  else
    fail "NetworkPolicy must not render when networkPolicy.enabled=false"
  fi

  if grep -q '^  replicas: 1$' /tmp/helm-standard.yaml &&
     ! grep -q '^kind: HorizontalPodAutoscaler$' /tmp/helm-standard.yaml; then
    pass "Disabled autoscaling keeps the fixed replica count and renders no HPA"
  else
    fail "Default autoscaling behavior was not preserved"
  fi
...
# Test 10: NetworkPolicy
test_network_policy() {
  section "Test 10: NetworkPolicy configuration"

  if helm template policy-release "$SCRIPT_DIR" \
    --namespace cased-system \
    --set networkPolicy.enabled=true \
    > /tmp/helm-network-policy.yaml 2>&1; then
    pass "Enabled NetworkPolicy renders successfully"
  else
    fail "Enabled NetworkPolicy failed to render"
    cat /tmp/helm-network-policy.yaml
    echo ""
    return
  fi

  if grep -q '^kind: NetworkPolicy$' /tmp/helm-network-policy.yaml &&
     grep -q '^  name: policy-release-cased-cd$' /tmp/helm-network-policy.yaml &&
     grep -q '^  namespace: cased-system$' /tmp/helm-network-policy.yaml; then
    pass "NetworkPolicy uses the release name and namespace"
  else
    fail "NetworkPolicy metadata was not rendered correctly"
  fi

  if grep -A5 '^  podSelector:$' /tmp/helm-network-policy.yaml | grep -q 'app.kubernetes.io/name: cased-cd' &&
     grep -A5 '^  podSelector:$' /tmp/helm-network-policy.yaml | grep -q 'app.kubernetes.io/instance: policy-release'; then
    pass "NetworkPolicy selects only this release's Cased CD pods"
  else
    fail "NetworkPolicy pod selector does not match the Deployment"
  fi

  if grep -A2 '^  policyTypes:$' /tmp/helm-network-policy.yaml | grep -q -- '- Ingress' &&
     grep -A4 '^  ingress:$' /tmp/helm-network-policy.yaml | grep -q -- '- port: http' &&
     ! grep -q -- '- Egress' /tmp/helm-network-policy.yaml &&
     ! grep -q '^  egress:' /tmp/helm-network-policy.yaml; then
    pass "NetworkPolicy limits ingress to HTTP without overriding egress policies"
  else
    fail "NetworkPolicy ingress or egress rules were not rendered correctly"
  fi

  echo ""
}

# Test 11: Horizontal Pod Autoscaler
test_autoscaling() {
  section "Test 11: Horizontal Pod Autoscaler configuration"

  if helm template autoscaled "$SCRIPT_DIR" \
    --namespace cased-system \
    --set autoscaling.enabled=true \
    --set autoscaling.minReplicas=2 \
    --set autoscaling.maxReplicas=7 \
    --set autoscaling.targetCPUUtilizationPercentage=65 \
    --set autoscaling.targetMemoryUtilizationPercentage=75 \
    > /tmp/helm-autoscaling.yaml 2>&1; then
    pass "Enabled autoscaling renders successfully"
  else
    fail "Enabled autoscaling failed to render"
    cat /tmp/helm-autoscaling.yaml
    echo ""
    return
  fi

  if grep -q '^apiVersion: autoscaling/v2$' /tmp/helm-autoscaling.yaml &&
     grep -q '^kind: HorizontalPodAutoscaler$' /tmp/helm-autoscaling.yaml &&
     grep -q '^  name: autoscaled-cased-cd$' /tmp/helm-autoscaling.yaml &&
     grep -q '^  namespace: cased-system$' /tmp/helm-autoscaling.yaml; then
    pass "HPA uses autoscaling/v2 and the release metadata"
  else
    fail "HPA API version or metadata was not rendered correctly"
  fi

  if grep -A4 '^  scaleTargetRef:$' /tmp/helm-autoscaling.yaml | grep -q 'apiVersion: apps/v1' &&
     grep -A4 '^  scaleTargetRef:$' /tmp/helm-autoscaling.yaml | grep -q 'kind: Deployment' &&
     grep -A4 '^  scaleTargetRef:$' /tmp/helm-autoscaling.yaml | grep -q 'name: autoscaled-cased-cd' &&
     grep -q '^  minReplicas: 2$' /tmp/helm-autoscaling.yaml &&
     grep -q '^  maxReplicas: 7$' /tmp/helm-autoscaling.yaml; then
    pass "HPA targets the release Deployment with configured replica bounds"
  else
    fail "HPA scale target or replica bounds were not rendered correctly"
  fi

  if grep -A5 '^        name: cpu$' /tmp/helm-autoscaling.yaml | grep -q 'averageUtilization: 65' &&
     grep -A5 '^        name: memory$' /tmp/helm-autoscaling.yaml | grep -q 'averageUtilization: 75'; then
    pass "HPA renders configured CPU and memory utilization targets"
  else
    fail "HPA resource metrics were not rendered correctly"
  fi

  if ! grep -q '^  replicas:' /tmp/helm-autoscaling.yaml; then
    pass "Autoscaled Deployment has no fixed replica count"
  else
    fail "Deployment must omit spec.replicas while autoscaling is enabled"
  fi

  if helm template cpu-only "$SCRIPT_DIR" \
    --set autoscaling.enabled=true \
    --set autoscaling.targetMemoryUtilizationPercentage=null \
    > /tmp/helm-autoscaling-cpu.yaml 2>&1 &&
     grep -q '^        name: cpu$' /tmp/helm-autoscaling-cpu.yaml &&
     ! grep -q '^        name: memory$' /tmp/helm-autoscaling-cpu.yaml; then
    pass "Either utilization metric can be omitted independently"
  else
    fail "CPU-only autoscaling did not render correctly"
  fi

  if helm template no-metrics "$SCRIPT_DIR" \
    --set autoscaling.enabled=true \
    --set autoscaling.targetCPUUtilizationPercentage=null \
    --set autoscaling.targetMemoryUtilizationPercentage=null \
    > /tmp/helm-autoscaling-invalid.log 2>&1; then
    fail "Autoscaling must reject a configuration with no utilization target"
  elif grep -q 'requires at least one CPU or memory utilization target' \
      /tmp/helm-autoscaling-invalid.log; then
    pass "Autoscaling rejects a configuration with no utilization target"
  else
    fail "Missing-target autoscaling failed without the expected validation message"
  fi

  echo ""
}

# Test 12: Published standalone manifest
test_install_manifest() {
  section "Test 12: Standalone install manifest"
